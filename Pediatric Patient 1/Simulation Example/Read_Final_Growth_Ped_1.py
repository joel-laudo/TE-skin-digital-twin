...
pts = np.loadtxt('Ped_1_Skin_Integration_points.txt')
pts = np.array(pts)
pts_len = len(pts)
...
for tmp in range(num_sim):
    ...
    if tmp == 1000:
        ...
        for increment in increments:
            ...
            np.savetxt('Ped_1_thg_raw_intPt_data_Sim_' +
                       str(tmp+1) + '_MAP_V4.txt', thetag, fmt='%.6f')

            np.savetxt('Ped_1_thg_elem_data_Sim_' + str(tmp+1) +
                       '_MAP_V4.txt', thetagElemValues, fmt='%.6f')
