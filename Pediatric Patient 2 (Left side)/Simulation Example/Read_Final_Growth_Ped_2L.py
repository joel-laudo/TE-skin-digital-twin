...
pts = np.loadtxt('Ped_2L_int_pt_coords.txt')
pts = np.array(pts)
pts_len = len(pts)
...
for tmp in range(num_sim):
    ...
    if tmp == 1000:
        ...
        odbPath = 'Ped_2L_ExpJobMAPV2.odb'
        ...
        for increment in increments:
            ...
            np.savetxt('Ped_2L_thg_raw_intPt_data_Sim_' +
                       str(tmp+1) + '_MAPV2.txt', thetag, fmt='%.6f')

            np.savetxt('Ped_2L_thg_elem_data_Sim_' + str(tmp+1) +
                       '_MAPV2.txt', thetagElemValues, fmt='%.6f')
