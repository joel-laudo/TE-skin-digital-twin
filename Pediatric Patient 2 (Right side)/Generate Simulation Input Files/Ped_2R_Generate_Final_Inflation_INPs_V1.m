...
%% Load Base INP file
fileID = fopen('Ped_2R_Final_Protocol_Needs_Edit_V2.inp','r');
...
% AddRowPara=11;
AddRowExpNodes = 132321; %adds shifted and rotated expander nodes
AddRowNode9999 = 142330; %adds the new node 9999 after shifting
AddRowSpringStiffness = 142519; %adds spring stiffness information
AddRowFixed=142520; %adds fixed skin nodes



%% Load Skin Nodes and Skin Elements
Skin_Nodes = readmatrix("Ped_2R_INPUT_Skin_Nodes_REMESH_V2.txt");
Skin_Elements = readmatrix("Ped_2R_INPUT_Skin_Elements_REMESH_V2.txt");
...
expd_pts_and_normals = readmatrix("Ped_2R_ExpanderSkin_points_with_normals_V2.xyz", "FileType","text");
%% Load mu, k, distance of expander
designs=load("Ped_2R_MAP_design.txt");

%% Job 1-100,
for JobNum=1:1
    tol = designs(JobNum,2);
    offset = designs(JobNum, 3) + 3.6; %Adjust for skin thickness (difference between top and bottom layers)
    mu = designs(JobNum, 4);
    tcrit = designs(JobNum, 5);
    kk = designs(JobNum, 6);
    theta = 0; %No expander rotation
    fname='FINAL_Ped_2R_Job'+string(JobNum)+'_MAP_V3.inp';
    fid = fopen(fname, 'w' );
    ...
    [new_node_9999, NewExpNodes] = Ped_2R_UpdateExpanderNodes_V1(original_mesh_nodes, original_mesh_elements, expd_pts_and_normals, offset, theta);
    ...
    % Add BC
    Fixed_Skin_Nodes=Ped_2R_FixSkinNodes_V1(tol, Skin_Nodes, Skin_Elements, NewExpNodes);
