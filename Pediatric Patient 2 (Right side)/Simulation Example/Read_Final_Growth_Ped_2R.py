...
pts = np.loadtxt('Ped_2R_int_pt_coords_V3.txt')
pts = np.array(pts)
pts_len = len(pts)
...
for tmp in range(num_sim):
    ...
    if tmp == 1000:
        ...
        for increment in increments:
            ...
            np.savetxt('Ped_2R_thg_raw_intPt_data_Sim_' +
                       str(tmp+1) + '_MAPV3.txt', thetag, fmt='%.6f')

            np.savetxt('Ped_2R_thg_elem_data_Sim_' + str(tmp+1) +
                       '_MAPV3.txt', thetagElemValues, fmt='%.6f')
